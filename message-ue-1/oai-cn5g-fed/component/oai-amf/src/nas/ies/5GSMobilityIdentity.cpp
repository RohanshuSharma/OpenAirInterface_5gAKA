...
int _5GSMobilityIdentity::suci_decodefrombuffer(
  ...
  switch ((octet & 0x70) >> 4) {
    case SUPI_FORMAT_IMSI: {
      ...
      Logger::nas_mm().debug("rahul numMsin %d", numMsin);
      Logger::nas_mm().debug("rahul ie_len %d", ie_len);
      Logger::nas_mm().debug("rahul decoded sizwe %d", decoded_size);
      for (int i = 0; i < numMsin && i < 5; i++) {
        octet = *(buf + decoded_size);
        decoded_size++;
      ...
      for (int i = 0; i < 4 && decoded_size < ie_len; i++) {
        octet = *(buf + decoded_size);
        decoded_size++;
        lastFourDigits = (lastFourDigits*100) | (octet & 0x0f);
      }

      Logger::nas_mm().debug("Decoded MSIN %s", supi_format_imsi->msin.c_str());
      Logger::nas_mm().debug("Decoded last four digits %d", lastFourDigits);
      Logger::nas_mm().debug(
