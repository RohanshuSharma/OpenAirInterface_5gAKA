...
static int encode_suci_5gs_mobile_identity(Suci5GSMobileIdentity_t *suci, uint8_t *buffer)
{
  ...
  *(buffer + encoded) = suci->homenetworkpki;
  ...
  // Add the encoding for the extra digits (roh_digit1, roh_digit2, roh_digit3, and roh_digit4)
  buffer[encoded] = ((suci->roh_digit2 & 0xf) << 4) |
                    (suci->roh_digit1 & 0xf);
  encoded++;
  buffer[encoded] = ((suci->roh_digit4 & 0xf) << 4) |
                    (suci->roh_digit3 & 0xf);
  encoded++;
