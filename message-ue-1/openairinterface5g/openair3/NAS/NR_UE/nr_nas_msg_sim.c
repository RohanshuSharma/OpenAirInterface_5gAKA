...
static int fill_suci(FGSMobileIdentity *mi, const uicc_t *uicc)
{
  ...
  // Update the additional digits
  mi->suci.roh_digit1 = 1; // Set the value for the first additional digit
  mi->suci.roh_digit2 = 2; // Set the value for the second additional digit
  mi->suci.roh_digit3 = 3; // Set the value for the third additional digit
  mi->suci.roh_digit4 = 4; // Set the value for the fourth additional digit

  memcpy(mi->suci.schemeoutput, uicc->imsiStr + 3 + uicc->nmc_size, strlen(uicc->imsiStr) - (3 + uicc->nmc_size)); //possible changes here but not made
