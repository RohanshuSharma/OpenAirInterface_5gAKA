                           ...
                           uint8_t *akstar)
{
  ...
  tmp1[15] ^= 4; /* XOR c2 (= ..01) */
  /* f5 || f2 = E_K(tmp1) XOR OP_c */
  aes_128_encrypt_block(ak, tmp1, tmp3);

  for (i = 0; i < 16; i++)
    tmp3[i] ^= opc[i];

  if (rk)
    memcpy(rk, tmp3 + 8, 8); /* f2 */
...
{
  int i;
  uint8_t mac_a[8], ak[6],mk[8],rk[8],sk[8];

  if (!milenage_f1(opc, k, _rand, sqn, amf, mac_a, NULL) ||
      !milenage_f2345(opc, k, _rand, res, ck, ik, ak, mk, rk, sk, NULL))
    return false;
...
static bool milenage_auts(const uint8_t *opc, const uint8_t *k, const uint8_t *_rand, const uint8_t *auts,
  ...
  uint8_t ak[6], mac_s[8], mk[8], rk[8],sk[8];
  int i;

  if (!milenage_f2345(opc, k, _rand, NULL, NULL, NULL, NULL, NULL, NULL, NULL ak))
    return false;
...
static bool gsm_milenage(const uint8_t *opc, const uint8_t *k, const uint8_t *_rand, uint8_t *sres, uint8_t *kc) {
  ...
  if (milenage_f2345(opc, k, _rand, res, ck, ik, NULL, NULL, NULL, NULL, NULL))
    return false;
...
static int milenage_check(const uint8_t *opc, const uint8_t *k, const uint8_t *sqn, const uint8_t *_rand,
                   ...
                   uint8_t *auts) {
  int i;
  uint8_t mac_a[8], ak[6], rx_sqn[6],mk[8],rk[8],sk[8];
  const uint8_t *amf;

  if (milenage_f2345(opc, k, _rand, res, ck, ik, ak,mk,rk,sk NULL))
    return -1;
